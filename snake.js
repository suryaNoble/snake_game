...
function handleTouch(event) {
    ...
    if (touchY < canvasHeight / 3 && direction.y === 0) {
        direction = { x: 0, y: -1 };  // Moving Up
    } else if (touchY > (2 * canvasHeight) / 3 && direction.y === 0) {
        direction = { x: 0, y: 1 };   // go Down
    } else if (touchX < canvasWidth / 3 && direction.x === 0) {
        direction = { x: -1, y: 0 };  // Move Left
    } else if (touchX > (2 * canvasWidth) / 3 && direction.x === 0) {
        direction = { x: 1, y: 0 };   // take Right
    }
}
